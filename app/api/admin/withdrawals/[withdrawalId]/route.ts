...
export async function PATCH(
  ...
  if (action === "REVIEW") {
    ...
    const updated = await prisma.withdrawalRequest.update({ where: { id: withdrawalId }, data: { status: WithdrawalStatus.REVIEWING, reviewNotes: notes ?? null } });
  ...
  if (action === "APPROVE") {
    ...
    const updated = await prisma.withdrawalRequest.update({ where: { id: withdrawalId }, data: { status: WithdrawalStatus.APPROVED, reviewNotes: notes ?? null } });
    return NextResponse.json({ status: updated.status });
  }
  ...
  if (action === "REJECT") {
    ...
    await prisma.$transaction([
      prisma.withdrawalRequest.update({ where: { id: withdrawalId }, data: { status: WithdrawalStatus.REJECTED, reviewNotes: notes ?? null, processedAt: new Date() } }),
      prisma.ledgerEntry.updateMany({ where: { reference: withdrawalId, status: LedgerEntryStatus.RESERVED }, data: { status: LedgerEntryStatus.REVERSED } }),
    ]);
  ...
  if (action === "PROCESS") {
    ...
    const updated = await prisma.withdrawalRequest.update({ where: { id: withdrawalId }, data: { status: WithdrawalStatus.PROCESSING, providerRef: providerRef ?? withdrawal.providerRef } });
    return NextResponse.json({ status: updated.status });
  }
  ...
  await prisma.$transaction([
    prisma.withdrawalRequest.update({ where: { id: withdrawalId }, data: { status: WithdrawalStatus.PAID, providerRef: providerRef ?? withdrawal.providerRef, processedAt: new Date() } }),
    prisma.ledgerEntry.updateMany({ where: { reference: withdrawalId, status: LedgerEntryStatus.RESERVED }, data: { status: LedgerEntryStatus.SETTLED, settledAt: new Date() } }),
  ]);
