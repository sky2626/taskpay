...
const navItems = [
  ...
  { label: "Reviews", href: "/business/reviews", icon: ClipboardCheck },
  ...
  return (
    <aside className="hidden w-64 shrink-0 border-r border-white/10 bg-slate-950/55 px-4 py-6 backdrop-blur-xl lg:flex lg:flex-col">
      <Link href="/" className="mb-8 px-3">
        ...
        <p className="mt-1 flex items-center gap-2 text-lg font-semibold text-white"><Building2 className="h-5 w-5" /> Business</p>
      </Link>
      ...
      <nav className="space-y-1">
        {navItems.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={`flex items-center gap-3 rounded-2xl px-3 py-3 text-sm transition ${active ? "bg-violet-500/15 text-violet-200 ring-1 ring-violet-400/20" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}>
      ...
      <div className="mt-auto border-t border-white/10 pt-5">
        <Link href="/business/settings" className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm text-slate-400 hover:bg-white/5 hover:text-white"><Settings className="h-4 w-4" /> Settings</Link>
      </div>
    </aside>
  );
}
